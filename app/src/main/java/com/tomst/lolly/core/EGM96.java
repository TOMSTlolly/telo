    ...
    public boolean isGridAvailable(String path) {
        ...
        try {
            if (path.startsWith("content"))
                gridFolder = DocumentFile.fromTreeUri(LollyActivity.getInstance(), Uri.parse(path));
        ...
        public void run() {
            ...
            if (!isPrivateDocumentPresent && isSharedDocumentPresent) {
                ...
                if (sharedDocument.exists()) {
                    ...
                    OutputStream out;
                    try {
                        in = LollyActivity.getInstance().getContentResolver().openInputStream(sharedDocument.getUri());
                        out = LollyActivity.getInstance().getContentResolver().openOutputStream(privateDocument.getUri());
                        copyFile(in, out);
                        in.close();
                        in = null;
                        out.flush();
            ...
            if (isPrivateDocumentPresent) {
                ...
                InputStream fin;
                try {
                    fin = LollyActivity.getInstance().getContentResolver().openInputStream(privateDocument.getUri());
